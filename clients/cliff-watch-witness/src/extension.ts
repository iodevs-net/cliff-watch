...
import {
    ...
    EDIT_BURST_TIMEOUT_MS,
    EDIT_BURST_PASTE_THRESHOLD,
    HEARTBEAT_INTERVAL_MS
} from './config';
import { ConfigurationManager, CliffWatchConfig } from './configuration';

let metricsEngine: MetricsEngine;
let configManager: ConfigurationManager;
let metricsDisplayInterval: ReturnType<typeof setInterval> | null = null;
 ...
 */
function processEvent(event: SensorEvent) {
    const config = configManager.getConfig();
    if (!config.enabled) {
        return;
    }
 ...
 */
function handleConfigChange(newConfig: CliffWatchConfig) {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('cliffWatchWitness.showMetrics', showDetailedMetrics)
    );
    ...
    context.subscriptions.push(
        vscode.workspace.onDidChangeTextDocument((e) => {
            ...
            const is_likely_paste = e.contentChanges.some(c => c.text.length > PASTE_THRESHOLD_CHARS);
    ...
    function flushEditBurst() {
        ...
        const event: SensorEvent = {
            ...
            timestamp_ms: now(),
            metadata: {
                is_likely_paste: editBurstAccumulator > EDIT_BURST_PASTE_THRESHOLD // Si la ráfaga acumulada es muy grande
            }
        };
        processEvent(event);
    ...
    context.subscriptions.push(
        vscode.window.onDidChangeTextEditorVisibleRanges((e) => {
            ...
            if (currentTime - lastScrollTime < SCROLL_THROTTLE_MS) return; // Throttle 1s
    ...
    }, HEARTBEAT_INTERVAL_MS); // 15s para mayor resolución en el MetricsEngine
    ...
    context.subscriptions.push({ dispose: () => {
        if (metricsDisplayInterval) clearInterval(metricsDisplayInterval);
    }});
...
export function deactivate() {
    processEvent({ type: 'disconnect', timestamp_ms: now() });
    if (metricsEngine) {
        metricsEngine.flush();
    }
    if (statusBarItem) {
